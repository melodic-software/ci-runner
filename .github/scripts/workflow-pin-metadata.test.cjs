...
const ciWorkflowsSha = "7107b34832a7b6db5d08d3b132621c599fbe5e50";
const ciWorkflowsVersion = "v0.14.2";
...
test("ci-workflows references use a full SHA with one release version", () => {
  ...
  assert.equal(references[0], ciWorkflowsSha, "ci-workflows must use the reviewed v0.14.2 SHA");
  assert.equal(
    new Set(versions).size,
    1,
    "ci-workflows references must name one release version for online pin verification",
  );
  assert.equal(versions[0], ciWorkflowsVersion, "ci-workflows must identify release v0.14.2");
});
