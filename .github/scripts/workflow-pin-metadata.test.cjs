...
const ciWorkflowsSha = "036c3909885da6efaff7e76b676d97ae9450067d";
const ciWorkflowsVersion = "v0.13.0";
...
test("ci-workflows references use a full SHA with one release version", () => {
  ...
  assert.equal(references[0], ciWorkflowsSha, "ci-workflows must use the reviewed v0.13.0 SHA");
  assert.equal(
    new Set(versions).size,
    1,
    "ci-workflows references must name one release version for online pin verification",
  );
  assert.equal(versions[0], ciWorkflowsVersion, "ci-workflows must identify release v0.13.0");
});
