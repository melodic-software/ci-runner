...
const ciWorkflowsSha = "e94438746c300b02385a7f8a2a2dcd19a7f4ad4a";
const ciWorkflowsVersion = "v0.10.2";
...
test("ci-workflows references use a full SHA with one release version", () => {
  ...
  assert.equal(references[0], ciWorkflowsSha, "ci-workflows must use the reviewed v0.10.2 SHA");
  assert.equal(
    new Set(versions).size,
    1,
    "ci-workflows references must name one release version for online pin verification",
  );
  assert.equal(versions[0], ciWorkflowsVersion, "ci-workflows must identify release v0.10.2");
});
