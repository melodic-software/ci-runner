...
`ci-runner` is a single Go module (`github.com/melodic-software/ci-runner`, `go 1.26.5`)
...
The base `go` on `PATH` is 1.22.2. Go's `GOTOOLCHAIN=auto` reads the `go 1.26.5`
directive in `go.mod` and transparently fetches/execs the go1.26.5 toolchain when
you run `go` from the repo, so `go version` reports 1.26.5 here. Nothing extra is
...
Go and refuse to run against this go1.26.5 module with
`the Go language version (goX.Y) used to build golangci-lint is lower than the
targeted Go version (1.26.5)`. You must build golangci-lint from source with the
repo's toolchain (it is not part of the startup update script):

```bash
GOTOOLCHAIN=go1.26.5 GOFLAGS=-mod=mod go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.6.0
golangci-lint run ./...
```

`GOTOOLCHAIN=go1.26.5` is required — without it, `go install` honors
