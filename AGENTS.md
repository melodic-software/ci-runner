...
The base `go` on `PATH` is 1.22.2. Go's `GOTOOLCHAIN=auto` reads the `go 1.27.1`
directive in `go.mod` and transparently fetches/execs the go1.27.1 toolchain when
you run `go` from the repo, so `go version` reports 1.27.1 here. Nothing extra is
...
targeted Go version (1.27.1)`. You must build golangci-lint from source with the
