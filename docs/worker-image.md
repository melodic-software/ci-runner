...
versioned language toolchains (`dotnet`, `node`, `npm`, `npx`, `python`,
`python3`, `pip`, `pip3`) are absent from PATH so workflows must install them
with their official setup actions. The runner's internal `externals/node*`
runtimes are not a workflow `node` on PATH.
